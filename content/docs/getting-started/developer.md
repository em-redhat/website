...
Both workflows enforce branch conventions: Speckit uses `speckit/NNN-<short-name>` branches (created by `/speckit.specify`), and OpenSpec uses `opsx/<change-name>` branches (created by `/opsx-propose`). Branch validation is a hard gate at each pipeline step. Speckit commands continue to accept legacy `NNN-<short-name>` branches.
